...
/// <summary>
/// Chứa toàn bộ Sub-Patterns và Compiled Regex chuẩn hóa từ bộ quy tắc lọc tin nhắn bất động sản & bán hàng
/// </summary>
public static class FilterRegexPatterns
{
    ...
    public static class Patterns
    {
        ...
        // Thời hạn hợp đồng / hạn thuê (VD: 12th, _12th, 6-12m, HĐ 6-12T, hd 30/7/2027, hạn đến 31/8/2027)
        public const string Duration = @"(?:[-–—_]?[ \t]*(?:(?:hd|HĐ|hạn|Hạn|hợp[ \t]*đồng|thời[ \t]*hạn)[ \t]*(?:tới|toi|đến|den)?[ \t]*[\d\/\.\-–—_]+(?:[ \t]*[-–—_][ \t]*[\d\/\.]+)*[ \t]*(?:[mM]|[tT]|[tT][hH]|[tT][hH]á[nN][gG]|[nN]ă[mM])?|[\d\/\.\-–—_]+(?:[ \t]*[-–—_][ \t]*[\d\/\.]+)*[ \t]*(?:[mM]|[tT]|[tT][hH]|[tT][hH]á[nN][gG]|[nN]ă[mM])))";

        // Ghi chú đính kèm trong ngoặc đơn hoặc dấu gạch ngang
        public const string NoteBracket = @"(?:\([ \t]*[^\n)]*\)|(?:[+&,; \t–—_\/-]*)(?:[cC]hủ[ \t]*dẫn|[cC]tv[ \t]*dẫn|[cC][tT][vV]|[cC][dD]|[cC]hốt|[cC]hốt[ \t]*ở|[fF]ix|[fF]ix[ \t]*giá|[kK]hách[ \t]*dẫn|[tT]hưởng[ \t]*(?:[nN]óng[ \t]*)?(?:[sS]ale|[cC]tv)?|[bB]onus[ \t]*(?:[sS]ale|[cC]tv)?):?[ \t]*(?:\d{1,3}[ \t]*%|\d+(?:[\.,]\d+)?[ \t]*(?:tr|triệu|k)[ \t]*\d*)[ \t]*(?:[-–—_]?[ \t]*(?:hd|HĐ|hạn|Hạn|hợp[ \t]*đồng|thời[ \t]*hạn|AD|ad|Ad|áp[ \t]*dụng|ap[ \t]*dung|trước|tới|toi|đến|den)[ \t]*[\d\/\.\-–—_]+(?:[ \t]*[-–—_][ \t]*[\d\/\.]+)*[ \t]*(?:[mM]|[tT]|[tT][hH]|[tT][hH]á[nN][gG]|[nN]ă[mM])?)?)?";

        // Giá trị tỷ lệ % có ký hiệu % (VD: 30%, 30-40%, 30%-40%, 30/40%)
        public const string PercentWithSymbol = @"(?:\d{1,3}[ \t]*(?:%[ \t]*)?[-–—_\/][ \t]*\d{1,3}[ \t]*%|\d{1,3}[ \t]*%)";

        // Giá trị tỷ lệ khoảng không có ký hiệu % (VD: 30-40, 30/40)
        public const string PercentRangeNoSymbol = @"(?:\d{1,3}[ \t]*[-–—_\/][ \t]*\d{1,3})";

        // Mốc hoa hồng phần trăm đơn lẻ / dải hoàn chỉnh
        public const string PercentSingle = @"(?:(?:" + PercentWithSymbol + @"[ \t]*" + Duration + @"?)|(?:" + PercentRangeNoSymbol + @"[ \t]+" + Duration + @"?)|(?:\d{1,3}(?:[ \t]+" + Duration + @"|[ \t]*(?=[+&,; \t–—_\/-]*(?:[cC]hủ[ \t]*dẫn|[cC]tv[ \t]*dẫn|[cC][tT][vV]|[cC][dD]|[cC]hốt|[fF]ix|[kK]hách|[tT]hưởng|[bB]onus)))))";
        ...
        public const string CommSegment = @"(?:(?:(?:" + HeaderPrefix + @"[ \t]*)?(?:" + PercentSingle + @")|(?:" + HeaderPrefix + @"[ \t]*" + MoneySingle + @"))[ \t]*(?:\([ \t]*[^\n)]*\)|(?:[+&,; \t–—_\/-]*)(?:[cC]hủ[ \t]*dẫn|[cC]tv[ \t]*dẫn|[cC][tT][vV]|[cC][dD]|[cC]hốt|[cC]hốt[ \t]*ở|[fF]ix|[fF]ix[ \t]*giá|[kK]hách[ \t]*dẫn|[tT]hưởng[ \t]*(?:[nN]óng[ \t]*)?(?:[sS]ale|[cC]tv)?|[bB]onus[ \t]*(?:[sS]ale|[cC]tv)?):?[ \t]*(?:\d{1,3}[ \t]*%|\d+(?:[\.,]\d+)?[ \t]*(?:tr|triệu|k)[ \t]*\d*)[ \t]*(?:[-–—_]?[ \t]*(?:hd|HĐ|hạn|Hạn|hợp[ \t]*đồng|thời[ \t]*hạn|AD|ad|Ad|áp[ \t]*dụng|ap[ \t]*dung|trước|tới|toi|đến|den)[ \t]*[\d\/\.\-–—_]+(?:[ \t]*[-–—_][ \t]*[\d\/\.]+)*[ \t]*(?:[mM]|[tT]|[tT][hH]|[tT][hH]á[nN][gG]|[nN]ă[mM])?)?)?)";
    ...
    public static readonly Regex CommissionRegex = new(
        @"(?:(?<=\n|^)[ \t]*)?" + Patterns.CommChain + @"[ \t]*(?:[\.\-–—_][ \t]*)?(?=[ \t]*[-([{:–—_ \t]*(?:(?:Mã|MÃ|mã):?|[🏆🎖️🥇⭐📍🏢☘⌛]|TL\d*|[hH]\d{2,4}\b)|[ \t]*(?:\n|$))",
    ...
    public static readonly Regex ProtectedLinePrefixRegex = new(
        @"^(?:(?:Mã|MÃ|mã):?|[🏆🎖️🥇⭐📍🏢☘⌛]|TL\d*|[hH]\d{2,4}\b|[pP]\d{2,4}\b)",
        RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.IgnoreCase,
        DefaultRegexTimeout
    );
