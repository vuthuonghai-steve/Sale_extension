...
> **Vị trí trong hệ thống:** Thuộc nhóm `Docs/Trade-off/frameworks/`. Được nạp on-demand khi chuẩn bị đưa ra quyết định kiến trúc, lựa chọn công nghệ, hoặc đánh giá rủi ro rollback.
...
| **Ví dụ trong dự án C# Native** | - Đổi cấu trúc `0_Contracts` (`IClipboardFilter`, `ClipboardDataPayload`).<br>- Chọn Win32 API Unmanaged thay vì Managed.<br>- Thay đổi chiến lược lưu trữ dữ liệu sang Database. | - Tối ưu biểu thức Regex trong `3_Modules/`.<br>- Tinh chỉnh thời gian Retry Backoff (10ms -> 20ms).<br>- Định dạng lại cấu trúc chuỗi log hiển thị. |
...
| **Yêu cầu tài liệu** | Bắt buộc có [`Docs/Trade-off/templates/adr-trade-off.template.md`](file:///c:/Users/ADMIN/Documents/workspace/Sale_extension/app_native_desktop/app_forms/Docs/Trade-off/templates/adr-trade-off.template.md). | Chỉ cần commit message rõ ràng và Unit Test bao phủ. |
...
1. **Level 1 (Sub-Module Local)**: Lỗi chỉ xảy ra bên trong 1 Filter (ví dụ: Regex Zalo bỏ sót ký tự). Hệ thống vẫn chạy, các filter khác vẫn hoạt động bình thường.
2. **Level 2 (Pipeline Engine)**: Lỗi làm treo chuỗi Pipeline hoặc mất dữ liệu văn bản cần copy.
3. **Level 3 (Process / App Crash)**: Lỗi ném Unhandled Exception làm tiến trình background biến mất khỏi System Tray.
