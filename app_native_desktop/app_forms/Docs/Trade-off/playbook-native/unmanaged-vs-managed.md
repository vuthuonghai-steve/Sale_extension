...
> **Ngữ cảnh áp dụng:** Khi thiết kế hoặc sửa đổi các Adapter giao tiếp với Win32 OS APIs (`user32.dll`, `kernel32.dll`), đặc biệt là đọc/ghi Windows Clipboard.

---

## 1. Bản Chất Bài Toán

Giao tiếp với Clipboard trong Windows Native đòi hỏi tương tác với bộ nhớ hệ thống thông qua `GlobalAlloc(GMEM_MOVEABLE)` và `SetClipboardData(CF_UNICODETEXT)`.
...
| **Nhược điểm (Pains)** | • Hay ném ngoại lệ ngầm `ExternalException` khi Clipboard bị lock mà không tự retry được.<br>• Buộc toàn bộ App phải phụ thuộc WinForms context. | • Phải tự quản lý bộ nhớ unmanaged.<br>• Nguy cơ Memory Leak nếu quên `GlobalFree` khi `SetClipboardData` thất bại. |
| **Vị trí áp dụng chuẩn** | Thử nghiệm nhanh (Prototyping). | **Sản phẩm chạy ngầm Production (Đang áp dụng trong `src/2_PlatformAdapters/`)**. |
...
1. **Bắt buộc dùng `try...finally`**:
   ...
   ```
2. **Không để rò rỉ Exception**: Luôn bọc trong khối an toàn trả về `bool` (Safe Method) và log mã lỗi.
