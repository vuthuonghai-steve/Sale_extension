...
(function () {
  ...
  const P = {
    ...
    // Đơn vị mốc hoa hồng đơn lẻ (VD: "40%- 12th" hoặc "30%- 6th")
    SINGLE_SEGMENT: `(?:\\d{1,3}[ \t]*%|\\d+(?:[\\.,]\\d+)?[ \t]*(?:tr|triệu|k)[ \t]*\\d*)[ \t]*(?:[-–—]?[ \t]*(?:hd|HĐ|hạn|Hạn|hợp[ \t]*đồng|thời[ \t]*hạn)?[ \t]*(?:tới|toi|đến|den)?[ \t]*[\\d\\/\\.\\-–—]+(?:[ \t]*[-–—][ \t]*[\\d\\/\\.]+)*[ \t]*(?:[mM]|[tT]|[tT][hH]|[tT][hH]á[nN][gG]|[nN]ă[mM])?)?`,

    // Phân cách giữa các mốc hoa hồng (dấu gạch đứng |, gạch ngang -, phẩy ,)
    SEGMENT_DELIMITER: `[ \t]*(?:[|/,\\-–—])[ \t]*`,
  ...
  const FilterRules = {
     ...
     * Nhận diện và loại bỏ các mẫu thông tin hoa hồng đứng dính trước Mã nhà, hỗ trợ đa mốc | và ghi chú ngoặc:
     * - "🌷 40%- 12th | 30%- 6th Mã: 🏆 379"
     ...
     * Xóa các icon hoa hồng mồ côi bị dư ở đầu dòng khi đứng trực tiếp trước Mã:
     * - "🌷 Mã: 🏆 063" -> "Mã: 🏆 063"
     */
     ...
     * - "30%-6th", "🌷 40%-12th", "/-rose 35%", "🌷35%-hd 31/8/2027"
     */
