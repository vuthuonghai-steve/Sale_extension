...
(function () {
  ...
  const P = {
    ...
    // Ghi chú đính kèm trong ngoặc đơn hoặc dấu gạch ngang (VD: "( ctv dẫn)", "( Chủ dẫn)", "( Chốt ở trước 15/7)", "(Chốt đúng giá, fix giá hh 30%)", "- CD 30%")
    ...
    NOTE_STANDALONE: `(?:\\([ \t]*.*?(?:[cC]hủ[ \t]*dẫn|[cC]tv[ \t]*dẫn|[cC][tT][vV]|[cC][dD]|[cC]hốt|[hH][hH]|[hH]oa[ \t]*hồng|[fF]ix[ \t]*giá|[fF]ix|[kK]hách[ \t]*dẫn).*?\\)|(?:[-–—_][ \t]*)?(?:[cC]hủ[ \t]*dẫn|[cC]tv[ \t]*dẫn|[cC][tT][vV]|[cC][dD]|[cC]hốt|[cC]hốt[ \t]*ở|[kK]hách[ \t]*dẫn):?[ \t]*(?:\\d{1,3}[ \t]*%|\\d+(?:[\\.,]\\d+)?[ \t]*(?:tr|triệu|k)[ \t]*\\d*|.*?))`,
