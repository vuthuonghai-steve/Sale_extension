...
(function () {
  ...
  window.ZaloQuickActionText = {
    ...
    removeSelectiveMetadata(text) {
      ...
      const NOTE_STANDALONE = `(?:\\([ \t]*.*?(?:[cC]hủ[ \t]*dẫn|[cC]tv[ \t]*dẫn|[cC][tT][vV]|[cC][dD]|[cC]hốt|[hH][hH]|[hH]oa[ \t]*hồng|[fF]ix|[fF]ix[ \t]*giá|[kK]hách[ \t]*dẫn|[tT]hưởng|[bB]onus|[hH]ỗ[ \t]*trợ|[tT]ặng).*?\\)|(?:[-–—_][ \t]*)?(?:[cC]hủ[ \t]*dẫn|[cC]tv[ \t]*dẫn|[cC][tT][vV]|[cC][dD]|[cC]hốt|[cC]hốt[ \t]*ở|[kK]hách[ \t]*dẫn|[tT]hưởng[ \t]*(?:[nN]óng[ \t]*)?(?:[sS]ale|[cC]tv)?|[bB]onus[ \t]*(?:[sS]ale|[cC]tv)?):?[ \t]*(?:\\d{1,3}[ \t]*%|\\d+(?:[\\.,]\\d+)?[ \t]*(?:tr|triệu|k)[ \t]*\\d*|.*?))`;
      ...
      const replyQuoteRegex = Rules.REPLY_QUOTE_REGEX || /^[ \t]*\[?[ \t]*(?:[tT]rả[ \t]*lời|[rR]eply|[tT]rích[ \t]*dẫn|[qQ]uote)[ \t]*:?[ \t]*[^\n]*\n(?:[ \t]*[^\n]*\n){1,3}(?=[ \t]*(?:(?:[hH][hH]|[hH]oa[ \t]*hồng|Mã|MÃ|mã|[-•\u1F300-\u1F9FF🌷🌸🌺🌻🌹💐🏢⌛☘🏆⭐📍]|\d|[a-zA-Z])))/gui;
      ...
      const commLineBonusRegex = Rules.COMMISSION_LINE_BONUS_REGEX || /(?:[tT]hưởng[ \t]*(?:[nN]óng[ \t]*)?(?:[sS]ale|[cC]tv|cho[ \t]*[sS]ale|cho[ \t]*[cC]tv|môi[ \t]*giới)|[bB]onus[ \t]*(?:[sS]ale|[cC]tv)|(?:[hH]ỗ[ \t]*trợ|[tT]ặng)[ \t]+(?:[sS]ale|[cC]tv)|[tT]hưởng[ \t]+[nN]óng|[tT]hưởng[ \t]+\d+(?:[\.,]\d+)?[ \t]*(?:k|tr|triệu|đ|vnd)?[ \t]*(?:\/[ \t]*(?:phòng|căn|p|hđ|hợp[ \t]*đồng))?[ \t]*(?:cho[ \t]*)?(?:sale|ctv|môi[ \t]*giới))/iu;
      ...
      result = result
        ...
        .filter(line => {
          ...
          // Bỏ dòng thông báo thưởng nóng / thưởng sale / bonus / chính sách độc lập (trừ khi dòng có chứa Mã/Địa chỉ/Thông tin phòng)
          if (commLineBonusRegex.test(trimmed) && !/^(?:(?:Mã|MÃ|mã):?|[🏆🎖️🥇⭐📍🏢☘⌛]|TL\d*)/iu.test(trimmed)) return false;
