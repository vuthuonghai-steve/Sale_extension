...
```mermaid
flowchart LR
    A -->|Right arrow label| B
    B <--|Left arrow label| A
    C -.->|Dashed labeled arrow| D
    E ---|"Text with spaces"| F
```

> **Warning**: Keep hyphen count exact! Use `-->|text|` or `<--|text|` (2 hyphens). Adding extra hyphens (e.g. `<---|text|`) is invalid Mermaid syntax.
> **Warning**: If link labels contain parentheses `()`, brackets `[]`, or special characters, you MUST wrap the label in double quotes inside the pipes (e.g. `A -->|"5. Parse (Fallback)"| B`). Unquoted parentheses inside `|...|` will break the parser with `Expecting 'SQE'... got 'PS'`.
