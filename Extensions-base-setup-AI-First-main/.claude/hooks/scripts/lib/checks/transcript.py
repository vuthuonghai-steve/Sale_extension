...
typecheck) chạy SAU nó không. Hỗ trợ 3 shape Antigravity (toolCall,
tool_calls, entry.name) + Claude Code (message.content tool_use); fail-open.
"""
...
_VERIFY_FIELD_NAMES = ("CommandLine", "commandLine", "cmd", "command")
_VERIFY_TOOL_NAMES = ("run_command", "Bash")


def _tool_calls(entry: dict) -> list[dict]:
    """Trích danh sách tool call từ một entry transcript theo các shape."""
...
def _is_verify_command(call: dict, verify_patterns: list[str]) -> bool:
    """Kiểm tra run_command/Bash có command khớp một verify pattern (substring)."""
    if call.get("name") not in _VERIFY_TOOL_NAMES:
        return False
    args = call.get("args")
