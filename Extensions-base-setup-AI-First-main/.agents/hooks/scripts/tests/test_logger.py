...
def _latest_log_file(tmp_path: Path) -> Path:
    logs_dir = tmp_path / ".agent" / "hooks" / "logs"
...
def test_log_never_raises_when_logs_dir_unwritable(tmp_path, monkeypatch, capsys):
    # .agent là file thường -> không thể tạo thư mục con -> log phải im lặng bỏ qua.
    broken_root = tmp_path / "broken"
    broken_root.mkdir()
    (broken_root / ".agent").write_text("i am a file, not a dir")
