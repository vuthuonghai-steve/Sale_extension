...
def repo_root() -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / ".agents").exists() or (candidate / ".agent").exists():
            return candidate
    return Path.cwd()
...
def load_rules() -> dict:
    ...
    repo_rules_agents = repo_root() / ".agents" / "hooks" / "scripts" / "config" / "rules.yaml"
    repo_rules_agent = repo_root() / ".agent" / "hooks" / "scripts" / "config" / "rules.yaml"

    for path in (real_rules, repo_rules_agents, repo_rules_agent):
        try:
